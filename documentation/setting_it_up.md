...
Once everything appear to be functioning, the next exciting step is [getting started](https://github.com/tebl/BulkyMIDI-32/blob/main/documentation/get_started.md) on using it to play some music. The easiest method by far is using a *USB to MIDI*-interface with [ScummVM](https://www.scummvm.org/), if only to verify that it all works by itself before attempting to hook it up to vintage equipment.